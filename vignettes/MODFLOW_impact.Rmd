...
round_to_string <- function(val, metric) {
  # Reference
  digits2 <- c("exceedance_level", "exceedance_range")
  digits1 <- c("stratification", "turtle_bay", "paddleboat", "motorboat", 
               "move_dock", "good_spawning")
  digits0 <- c("num_2yr")
...
```{r eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.width=3.5, fig.height=3.75}
metric   <- data.frame(metric = c("move_dock"),
                       ylabel = "Percent of Years")
variables <- data.frame(breaks = c("percent_moves"),
                        labels = c("Move Dock"))
