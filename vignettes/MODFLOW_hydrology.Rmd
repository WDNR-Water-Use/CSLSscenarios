...
```{r eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE}
plot_levels <- function(df, exceeds, scenarios, only_base = FALSE, text_size = 10) {
  if (only_base) {
    plot_obj <- ggplot(df)
  } else {
    plot_obj <- ggplot(df) +
...
```{r eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.width=6.5, fig.height=7}
plot_durations(MODFLOW, scenarios, variables = dur_var, only_no_irr = FALSE)
```
