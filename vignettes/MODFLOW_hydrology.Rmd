...
plot_durations <- function(df, scenarios, text_size = 10, base_sim = 1,
  ...
  plot_obj <- ggplot(data = durations,
              ...
              scale_x_continuous(expand = c(0,0),
                                 limits = c(0, max_months/12+1),
                                 breaks = seq(0,max_months/12,2),
                                 labels = c(as.character(seq(0,max_months/12-2,2)),
                                            sprintf(">%d", max_months/12)),
                                 minor_breaks = seq(0,max_months/12,1)) +
              facet_grid(variable~lake, switch = "y") +
...
Histograms for each scenario are overlayed on top of one another. Lake level
dynamics are MUCH less flashy than in our previous (historical) time series.
Levels do not really fluctuate about the median. Instead, they go down fast,
stay down, then come back up at the very end. Thus, there are very few
occurances of exceedance, and they tend to last a long time. It makes these
histograms kind of silly since there are so few occurrences - there may be a
better way to visualize this. To make them somewhat more readable, I binned by
year. Durations longer than 20 years include:

  1. Long Lake, Current Irrigation, Below Median: Nearly 29 years (345 months)
  2. Plainfield Lake, Current Irrigation, Below Median: Nearly 22 years (262
  months)

<br>

```{r eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.width=6.5, fig.height=7}
plot_durations(MODFLOW, scenarios, variables = dur_var)
```
...
extreme over 12 months with Current Irrigation at Long and Plainfield, but I
don't see these as meaningfully different.

<br>

```{r eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.width=6.5, fig.height=6}
plot_rates(MODFLOW, scenarios, variables = rate_var)
```
