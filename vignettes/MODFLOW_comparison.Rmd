...
MODFLOW         <- CSLSdata::MODFLOW %>%
                   filter(.data$scenario %in% c("no_irr", "irr"),
                          year(.data$date) >= 1986) %>%
...
MODFLOW_diff    <- dcast(MODFLOW_metrics,
                         ...
                         value.var = "value") %>%
                   mutate(value = .data$irr - .data$no_irr) 
...
plot_comparison <- function(df, 
                            ...
                            variables = data.frame(breaks = c("10", "25", "50",
                                                   ...
                                                   labels = c("Infrequent\nHigh",
                                                              ...
                                                              "Infrequent\nLow")),
                            series = "month",
                            colors = data.frame(breaks = c("no_irr", "irr"),
...
metric    <- data.frame(metric = "stratification",
                        ylabel = "Time Lake is Stratified (%)",
                        mTOft = FALSE)
variables <- data.frame(breaks = c("percent_strat"),
                        labels = c("% stratified"))
...
### Pleasant Lake - Turtle Bay

**Comparison**

```{r eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.width=3, fig.height=2.5}
metric    <- data.frame(metric = "turtle_bay",
                        ylabel = "Time Turtle Bay Connected (%)",
                        mTOft = FALSE)
variables <- data.frame(breaks = c("percent_connect",
...
### Pleasant Lake - Motorboat Area > 62.5 ac

**Comparison**

```{r eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.width=3, fig.height=2.5}
metric    <- data.frame(metric = "motorboat",
                        ylabel = "Percent of Time",
                        mTOft = FALSE)
variables <- data.frame(breaks = c("percent_ok"),
                        labels = c("Motorboating Legal"))
...
### Long Lake - Paddleboat basins connected & > 3ft

**Comparison**

```{r eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.width=3, fig.height=2.5}
metric    <- data.frame(metric = "paddleboat",
                        ylabel = "Percent of Time",
                        mTOft = FALSE)
variables <- data.frame(breaks = c("percent_ok"),
                        labels = c("Paddleboat Ok"))
...
metric    <- data.frame(metric = "move_dock",
                        ylabel = "Percent of Years",
                        mTOft = FALSE)
variables <- data.frame(breaks = c("percent_bad_june",
                                   "percent_moves"),
                        labels = c("Bad June Dock",
                                   "Move Dock"))
